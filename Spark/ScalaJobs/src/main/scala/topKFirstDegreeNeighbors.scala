...
object topKFirstDegreeNeighbors {
    def main(args: Array[String]) {

        // setup the Spark Context
        val conf = new SparkConf().setAppName("FindTopKFirstDegreeNeighbors")
        ...
        def putInHBase(vertex: (String, Array[Long]), k: Int): Unit = {
            ...
            // Top K 1st degree neighbors
            val topKNeighbors = getTopKNeighbors(vertex, k)
            val topKNeighborsString = "Top" + k.toString + "Neighbors"
            val topKNeighborsFamilyName = Bytes.toBytes(topKNeighborsString)
            val topKNeighborsQualifierName = Bytes.toBytes("FirstDegree")
            val topKNeighborsValue = Bytes.toBytes(topKNeighbors._2.mkString(","))
            putter.addColumn(topKNeighborsFamilyName, topKNeighborsQualifierName, topKNeighborsValue)
            table.put(putter)
            table.close()
        }
